...
class TitaniumAdapter {
	...
	exportDatabase(name, database, callback) {
		...
		let meta = _getMetaDescriptor(name)
		...
		for (let cidx = 0; cidx < database.collections.length; cidx++) {
			...
			let descriptor = _getPartitionDescriptor(name, cidx)
				.open(Titanium.Filesystem.MODE_WRITE);
	...
	loadDatabase(name, callback) {
		...
		for (let idx = 0; idx < database.collections.length; idx++) {
			...
			let descriptor = _getPartitionDescriptor(name, idx);
			...
			while (stream.read(buffer) > -1) {
				...
				offset = _chunk(offset, bytes, function (entry) {
					collection[idx].data.push(entry);
				});
...
/**
 * Retrieves a file descriptor for a metadata file.
 *
 * @param {String} name
 * 		the name of the database to locate for.
 */
function _getMetaDescriptor(name) {
	return Ti.Filesystem.getFile(ROOT, name + '-meta');
}

/**
 * Retrieves a file descriptor for a partition file.
 *
 * @param {String} name
 * 		the name of the database to locate for.
 * @param {Number} partition
 * 		the index of the partition to open for.
 */
function _getPartitionDescriptor(name, partition) {
	return Ti.Filesystem.getFile(ROOT, name + '-' + partition);
}
